...
Les générateurs peuvent, de façon automatique, implémenter certain types de fonctionnalités, vous faisant gagner du temps et évitant la possibilité d'avoir une certaine catégorie de bugs.

Bien qu'ayant beaucoup de partisans, les générateurs de code ont un coût qu'il faut prendre en compte avec attention avant de les inclure dans un projet.

Favorisez les générateus qui permettent une séparation claire entre fonctionnalité généré et non-générée, mais soyez sûr de comprendre les compromis que vous faîtes avant d'inclure un générateur dans votre projet..

## Détails

Les codes générateurs peuvent être classés entre trois groupes:

* Générateurs standards
* Processeur d'annotation à la compilation
* Générateurs/frameworks à l'éxécution


### Générateurs Standards

Les générateurs standards sont la forme la plus simple de la génération de code. Ils peuvent être divisés en:

* Les générateurs qui insèrent du code dans des classes éxistantes (càd les méthodes auto-générées par un IDE)
* Les générateurs qui 
